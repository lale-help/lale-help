...
# Create Admin User
admin = User.find_or_create_by(first_name: "Lale", last_name: "Admin", is_admin: true, primary_circle_id: circle.id, status: User.statuses[:active])
...
(1..5).each do |i|
  user = User.find_or_create_by(first_name: "Lale", last_name: "App#{i}", status: User.statuses[:active])
