...
describe Task::Comments::Updated do
  ...
  describe '#execute' do
    ...
    context "task changed" do
      ...
      it 'is successful' do
        outcome = Task::Comments::Updated.run(task: task, user: user)
        expect(outcome).to be_success
      end

      it 'creates a comment' do
        expect { Task::Comments::Updated.run(task: task, user: user) }.to change{ Comment.count }.by(1)
      end

      it "sets the correct values on the comment" do
        Task::Comments::Updated.run(task: task, user: user)
