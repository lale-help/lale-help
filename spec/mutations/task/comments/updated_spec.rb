...
describe Task::Comments::Updated do
  ...
  describe '#execute' do
    ...
    context "task changed" do
      ...
      it 'is successful' do
        outcome = Task::Comments::Updated.run(item: task, user: user, changes: task.changes)
        expect(outcome).to be_success
      end

      it 'creates a comment' do
        expect { Task::Comments::Updated.run(item: task, user: user, changes: task.changes) }.to change { Comment.count }.by(1)
      end

      it "sets the correct values on the comment" do
        Task::Comments::Updated.run(item: task, user: user, changes: task.changes)
